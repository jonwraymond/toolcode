...
func TestTools_DescribeTool_DelegatesToDocs(t *testing.T) {
	...
	if store.describeCalls[0].level != tooldocs.DetailFull {
...
func TestTools_RunTool_RecordsBackendKindOnToolError(t *testing.T) {
	backend := toolmodel.ToolBackend{Kind: toolmodel.BackendKindMCP}
	toolErr := toolrun.WrapError("tool1", &backend, "execute", errors.New("boom"))
	runner := &mockRunner{
		runErr: toolErr,
	}
	tools := newTools(&Config{
...
func TestTools_RunTool_RecordsBackendKind(t *testing.T) {
	testCases := []struct {
		...
		{"mcp", toolmodel.BackendKindMCP, "mcp"},
		{"provider", toolmodel.BackendKindProvider, "provider"},
		{"local", toolmodel.BackendKindLocal, "local"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			runner := &mockRunner{
				runResult: toolrun.RunResult{
					Backend: toolmodel.ToolBackend{Kind: tc.kind},
				},
			}
			tools := newTools(&Config{
				Index:  &mockIndex{},
...
func TestTools_RunChain_DelegatesToRunner(t *testing.T) {
	runner := &mockRunner{
		...
		chainSteps: []toolrun.StepResult{
			{Result: toolrun.RunResult{Structured: "step1"}},
			{Result: toolrun.RunResult{Structured: "step2"}},
		},
	}
	tools := newTools(&Config{
		Index:  &mockIndex{},
	...
	runner := &mockRunner{
		...
		chainSteps: []toolrun.StepResult{
			...
			},
			{
				Result: toolrun.RunResult{
					Structured: "result2",
					Backend:    toolmodel.ToolBackend{Kind: toolmodel.BackendKindLocal},
				},
			},
		},
	}
...
func TestTools_RunChain_BackendKindOnError(t *testing.T) {
	...
	_, _, err := tools.RunChain(context.Background(), []toolrun.ChainStep{{ToolID: "tool1"}})
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestTools_RunChain_CountsAgainstMaxToolCalls(t *testing.T) {
	...
	_, _, err := tools.RunChain(ctx, []toolrun.ChainStep{
		{ToolID: "tool1"},
		{ToolID: "tool2"},
	})
	if err != nil {
